...
bool PSAReader::Read()
{
	...
	while (!Ar.eof())
	{
		...
		else if (CHUNK("ANIMKEYS"))
		{
			...
			for (auto i = 0; i < DataCount; i++)
			{
				Ar.read(reinterpret_cast<char*>(&AnimKeys[i].Position), sizeof(FVector));
				Ar.read(reinterpret_cast<char*>(&AnimKeys[i].Orientation), sizeof(FQuat));
		...
		else if (CHUNK("SCALEKEYS"))
		{
			...
			for (auto i = 0; i < DataCount; i++)
			{
				Ar.read(reinterpret_cast<char*>(&ScaleKeys[i].ScaleVector), sizeof(FVector));
